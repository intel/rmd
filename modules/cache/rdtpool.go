...
//         resource group when calculating hospitality score.
//         Optional
...
func GetAvailableCacheSchemata(allres map[string]*resctrl.ResAssociation,
	...
	if pool == "none" {
		for k := range ReservedInfo[OS].Schemata {
			schemata[k], _ = BitmapsCacheWrapper(GetCosInfo().CbmMask)
		}
	} else {
	...
	for k, v := range allres {
		...
		if sv, ok := v.CacheSchemata[cacheLevel]; ok {
			...
			for _, cv := range sv {
				...
				bm, _ := BitmapsCacheWrapper(cv.Mask)
...
func GetCachePoolName(MaxWays, MinWays uint32) (string, error) {
	...
	return "", fmt.Errorf("max_cache=%d, min_cache=%d, doesn't support", MaxWays, MinWays)
}
