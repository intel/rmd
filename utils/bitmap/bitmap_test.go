...
func TestBitmapToString(t *testing.T) {
	mapList := []string{"1-8,^3-4,^7,9", "56-87,^86,^61-65"}
	b, _ := NewBitmap(88, mapList)
	str := b.ToString()
...
func TestGenCPUResStringSimple(t *testing.T) {
	mapList := []string{"0-7"}
	s, e := GenCPUResString(mapList, 88)
	if e != nil {
...
func TestGenCPUResStringWithWrongExpression(t *testing.T) {
	mapList := []string{"abc1-8,^3-4,^7,9", "56-87,^86,^61-65"}
	_, e := GenCPUResString(mapList, 88)
	if e != nil {
...
func TestString2data(t *testing.T) {
	hexDatas := []uint{0xffffff0f, 0xf1, 0xff2fff}
	...
	hexDatas = []uint{0x00ffafff, 0xdf00cfff, 0x3}
